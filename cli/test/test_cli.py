...
from tablassist.cli import list_categories, preview_csv, validate_config_file, validate_section_str
...
def test_validate_section_str_accepts_valid_fixture() -> None:
    result = validate_section_str((FIXTURES_DIR / "valid-config.yaml").read_text())

    assert result["status"] == "ok"


def test_validate_section_str_rejects_invalid_fixture() -> None:
    result = validate_section_str((FIXTURES_DIR / "invalid-config.yaml").read_text())
