...
2. For each candidate the scout returns, in order:
    ...
    c. Delegate to `the-extractor`: "Use `{topic_ledger_dir}/data/PMC{id}/` as the paper artifact root. The scout's `paper_url` for this PMC ID is `{paper_url}` — quote it verbatim if you need to cite a source URL. Download PMC{id} there, keep raw downloads under `raw/`, extracted source trees under `source/`, useful intermediate outputs under `derived/`, and disposable files under `scratch/`. When `download-pmc-oa` is used, capture the returned `s3_uri` so it can be recorded on the ledger entry. Inspect any tabular supplements with `describe-csv` / `describe-excel`, and report columns + paper context. Use `preview-*` only for narrow follow-up row checks. For each candidate identifier column, return 3-5 representative raw values with the proposed transforms (`explode_by` → `regex` → `remove` → `prefix`/`suffix`) pre-applied, plus the proposed `category`, `predicate`, `qualifiers[]`, and `taxon` (if any). Do not search CURIEs or look up Biolink docs — pioneer gates on those. If you clean anything up, only remove specific extractor-created scratch or temporary files for this paper; never run broad directory cleanup."
    d. If the extractor reports no usable tabular data, call `discovery-ledger add` with the appropriate status, the scout's `paper_url`, and continue:
       - `non-tabular-source` — the supplement was acquired but is PDF, DOCX, image, or otherwise not a usable tabular format.
       - `unobtainable-source` — the extractor exhausted the fallback chain without finding any real URL. **Never accept a fabricated or guessed URL.** If the extractor's failure report contains a "suggested URL" that was not directly observed in a real response, treat the source as unobtainable and skip — do not retry with the guessed URL.
       - `no-data` — fallback for any other reason no usable data was returned.
...
- Do not synthesize PMC IDs, paper titles, supplement filenames, or URLs the scout/extractor did not surface from real responses. If a candidate URL was not directly observed in a tool response, treat it as nonexistent.
- A skipped paper is a successful outcome. Forcing a config from a non-tabular or unobtainable source is a failure.
